...
function unfoldSurface(surface, treeImpl){
    var tree = new Tree(treeImpl);
    var vertices = [];
    ...
    var unfoldSubsurface = function(nodeIndex, parentIndex){
        ...
        // map subtree
        _.forEach(tree.getNode(nodeIndex).children, function(child){
            unfoldSubsurface(child.node, nodeIndex);
        });
    };
    unfoldSubsurface(tree.rootNode.id);
